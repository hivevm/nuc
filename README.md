...
building software **with coding agents** inside a ready-to-use Dev Container. The work is driven by
a written **specification** ([`docs/SPECIFICATION.md`](docs/SPECIFICATION.md)) and **Architecture
...
.claude/settings.json # Claude Code permissions: prompt before git/gh and before editing the
                      #   specification or an ADR; deny reading .env files
```

## Dev Container

The environment is defined entirely in
[`.devcontainer/devcontainer.json`](.devcontainer/devcontainer.json): it starts from a prebuilt base
image and layers Dev Container Features and VS Code extensions on top — no Dockerfile or Compose
file required. Customise the environment by adding Features, switching the base image, or adding
extensions.

### Host container management

The Dev Container deliberately has **no access to the host Docker daemon** — the socket is not
mounted ([ADR-0003](docs/adr/0003-dev-container-runtime.md)). To manage the host's containers from
...
lands, delete the pointer file rather than keeping a second file to maintain. Where a tool can
enforce a rule mechanically, its own configuration may do so:
[`.claude/settings.json`](.claude/settings.json) prompts before git and `gh` commands, and before
any edit to the specification or to an ADR — the two artifacts only a human decides
